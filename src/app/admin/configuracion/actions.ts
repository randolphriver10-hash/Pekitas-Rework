...
import { seoSettingsSchema, emptyToNull, type SeoSettingsInput } from "@/lib/validations/site";

export type ActionResult = { error?: string; conflict?: boolean } | undefined;

export async function updateSeoSettingsAction(
  input: SeoSettingsInput & { expectedUpdatedAt: string }
): Promise<ActionResult> {
  const { expectedUpdatedAt, ...rest } = input;
  const parsed = seoSettingsSchema.safeParse(rest);
  if (!parsed.success) {
