...
export async function upsertGalleryItemAction(input: GalleryItemInput): Promise<ActionResult> {
  ...
  const { error } = id
    ...
    : await supabase.from("gallery_items").insert({ ...payload, created_by: user?.id });
