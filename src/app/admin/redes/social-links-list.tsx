...
export function SocialLinksList({ links: initialLinks }: { links: SocialLinkRow[] }) {
  ...
  const handleReorder = (newLinks: SocialLinkRow[]) => {
    setLinks(newLinks);
    startTransition(async () => {
      const result = await reorderItemsAction("social_links", newLinks.map((l) => l.id), "/admin/redes");
      if (result?.error) toast.error(result.error);
    });
  };
      ...
      {links.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-lg border">
          <Table>
            ...
            <TableBody>
              <SortableList items={links} onReorder={handleReorder} strategy={verticalListSortingStrategy}>
                {(link) => (
                  <SortableItem key={link.id} id={link.id} as={TableRow}>
